  ...
  .then((querySnapshot) => {
    ...
    $("#FondoEditorial-Firebase").on("click", ".btnEditar", function () {
      ...
      let recepcionPropuesta =
        filaEditada["RECEPCIÓN DE LA PROPUESTA DE PUBLICACIÓN"];
      document.getElementById("6").value = recepcionPropuesta;

      let comunicacionAnalisisOriginalidad =
        filaEditada["COMUNICACIÓN DE ANÁLISIS DE ORIGINALIDAD"];
      ...
      let radicacionSolicitud3raEval =
        filaEditada["RADICACIÓN DE SOLICITUD CONTRATO TERCERA EVALUACIÓN"];
      ...
      let entregaLiderCorrecionEstilo =
        filaEditada["ENTREGA AL LÍDER DE ARCHIVOS CON CORRECCIÓN DE ESTILO"];
      document.getElementById("22").value = entregaLiderCorrecionEstilo;

      let archivosValidacionCorrecionEstilo =
        filaEditada["ARCHIVOS CON LA VALIDACIÓN DE LA CORRECCIÓN DE ESTILO"];
      ...
      let muestraContenidoDiseñado =
        filaEditada["MUESTRA DE CONTENIDO DISEÑADO"];
      ...
      let revisionEditorial =
        filaEditada["REVISIÓN EDITORIAL (VERIFICACIÓN EDITORIAL)"];
      ...
      let aclaracionDerechosObra =
        filaEditada["ACLARACIÓN DE DERECHOS SOBRE LA OBRA"];
      ...
      let textoAjustado =
        filaEditada["TEXTO AJUSTADO A PARTIR DEL ANÁLISIS DE ORIGINALIDAD"];
      document.getElementById("40").value = textoAjustado;

      let finalizaCorreccionEstilo =
        filaEditada["FINALIZA CORRECIÓN DE ESTILO"];
      ...
      let observacionAjustesCorreccion =
        filaEditada["OBSERVACIONES Y AJUSTES DE CORRECCIÓN"];
      ...
      let acuerdoTerminos =
        filaEditada["ACUERDO DE TERMINOS DE DISEÑO CON COORDINADORA EDITORIAL"];
